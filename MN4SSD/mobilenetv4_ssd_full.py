...
class AnchorGeneratorSSD(nn.Module):
    ...
    Returns per-level [H*W*A, 4] (cx, cy, w, h) in relative coordinates.
    """
    def __init__(
        self,
        strides: List[int],
    ...
    def grid_anchors(self, feat: torch.Tensor, level: int) -> torch.Tensor:
        ...
        # [h, w, A, 2]
        ctr = centers.view(h, w, 1, 2).expand(h, w, A, 2)
        wh = sizes.view(1, 1, A, 2).expand(h, w, A, 2)
...
class LevelHead(nn.Module):
    ...
    def forward(self, x):
        ...
        b, ac, h, w = cls.shape
        cls = (
            cls.view(b, self.num_classes, self.num_anchors, h, w)
               .permute(0, 3, 4, 2, 1)
               .contiguous()
        ...
        b, ar, h, w = reg.shape
        reg = (
            reg.view(b, 4, self.num_anchors, h, w)
               .permute(0, 3, 4, 2, 1)
               .contiguous()
               .view(b, -1, 4)
        )
...
class MobileNetV4SSD(nn.Module):
    ...
    def __init__(
        ...
        self.backbone_indices = list(
            range(len(chs) - feat_levels_from_backbone, len(chs))
        )
        ...
        self.strides = self._guess_strides()  # absolute strides wrt input
        self.scales = self._make_scales(num_levels)
        self.anchor_gen = AnchorGeneratorSSD(
            self.strides, img_size, self.scales, self.ratios
        )

    # --- Helpers for strides & scales
    def _guess_strides(self) -> List[int]:
        """
        Infer per-feature-map strides relative to the *input* resolution.
        For a square input of size S and feature height H_l, stride_l = S // H_l.
        """
        with torch.no_grad():
        ...
        for f in feats:
            h = int(f.shape[-2])
            if h <= 0:
                raise ValueError("Feature map has non-positive height, cannot infer stride.")
            stride = int(self.img_size // h)
