...
def eval_single_class(cls_id, preds, gt_by_img, num_gt, iou_thr, report_conf):
    ...
    gt_used = {
        img_id: np.zeros(len(per_cls.get(cls_id, [])), dtype=bool)
        for img_id, per_cls in gt_by_img.items()
        if cls_id in per_cls
    }
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="minimum confidence to keep detections before scoring",
    )
    ap.add_argument(
        "--report-conf",
        type=float,
        default=None,
        help="threshold used when reporting precision/recall (defaults to --conf)",
    )
    ...
    cat2contig = {
        cid: name2contig[id2name[cid]]
        for cid in id2name
        if id2name[cid] in name2contig
    }
    ...
    for idx, img_info in enumerate(images, 1):
        ...
        if args.no_letterbox:
            # Legacy warp: direct resize, no padding
            ...
            # Default: letterbox, consistent with training/inference
            im, scale_x, scale_y, pad_x, pad_y = letterbox_image(im0, args.img_size)
        ...
        # Undo letterbox / warp to original image coordinates
        sx = max(scale_x, 1e-6)
        sy = max(scale_y, 1e-6)
    ...
    for cls_id in range(1, num_classes):
        metrics = eval_single_class(
            cls_id,
            preds_per_class.get(cls_id, []),
            gt_by_img,
            gt_counts.get(cls_id, 0),
            args.iou,
            report_conf,
        )
    ...
    if recall.size > 0:
        plt.plot(
            recall,
            precision,
            label=f"micro AP@{args.iou:.2f} = {micro_ap:.3f}",
        )
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.xlabel("Recall")
    ...
    ap_values = []
    for idx_c, name in enumerate(names, start=1):
        m = class_metrics.get(idx_c)
        if m is None:
        ...
        rows.append(row)
        csv_lines.append(
            f"{name},{row[1]:.4f},{row[2]:.4f},{row[3]:.4f},{row[4]},{row[5]}"
        )
    ...
    print("-" * 72)
    print(
        f"{header[0]:15s} | {header[1]:>8s} | {header[2]:>8s} | "
        f"{header[3]:>8s} | {header[4]:>6s} | {header[5]:>6s}"
    )
    print("-" * 72)
    for row in rows:
        ...
        print(
            f"{name:15s} | {ap_s:>8s} | {p_s:>8s} | {r_s:>8s} | "
            f"{gt_count:6d} | {pred_count:6d}"
        )
    print("-" * 72)
    print(
        f"[summary] micro AP={micro_ap:.3f}  "
        f"macro AP={macro_ap:.3f}  GT boxes={total_gt}"
    )
